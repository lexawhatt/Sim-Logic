...
a button widget, drag manager, focus tree, or event consumption
...
`FrameInput::focus_lost()` reports the boundary even when no button or key was
held, so opening a pause menu does not depend on finding a cancellation edge.
It is false in later frames and outside FrameUpdate.
`FixedInput::focus_lost()` likewise reports it in every catch-up tick of the
same application frame, letting fixed systems stop cached movement before
FrameUpdate runs. It is false outside FixedUpdate and is not retained for a
later frame if this frame has no fixed tick; cancellation edges retain their
separate first-fixed-tick delivery described above.
...
## Relative motion and cursor capture

For a first-person camera, register the optional application resource once:

```rust,ignore
app.register_app_resource(PointerCapture::default())?;
```

A frame system can borrow `AppResMut<PointerCapture>` and call `request_lock()`
on a fresh click, or `release()` when opening a menu. The desktop adapter applies
the request after the logical frame. It tries a locked cursor first, then
confinement; either successful mode hides the cursor. `requested()` is the
application's intent; `status()` and `is_captured()` describe the last backend
acknowledgement. Unsupported capture returns `Unavailable`, keeps the cursor
visible and disables raw motion. An explicit new request can retry it; the
adapter does not retry every frame.

`FrameInput::relative_motion()` returns the accumulated `RelativePointerMotion`
for this frame. Its `x()` and `y()` are raw device displacement, with positive X
rightward and positive Y downward. They are not logical pixels. Apply sensitivity
once, without multiplying by elapsed time. Raw motion is accepted only while the
desktop window is focused and captured. Captured absolute cursor motion and
cursor-leave notifications are ignored, so lock-related warps do not become UI
movement or cancelled clicks.
When actual capture starts or ends, the adapter queues `PointerLeft` before
later button input and clears the old absolute position. This cancels unfinished
mouse gestures. Until a fresh `CursorMoved`, clicks have no position; releasing
capture must not let a click use stale coordinates from before mouse look.

Motion is frame-only: it is zero outside FrameUpdate, resets on the next frame,
and is never replayed in fixed catch-up ticks. Pause does not suppress it; the
application chooses whether its menu or camera should respond. A focus-loss
boundary discards all relative motion in that accepted frame, including later
motion in the same batch. Focus loss also clears capture intent before systems
run. Desktop release is immediate on focus loss, suspension, exit or host drop;
focus regain never requests capture automatically. Opening a menu must explicitly
release capture. A release failure reports `Unavailable`: visibility is restored
and raw input is disabled, but the OS grab state cannot be guaranteed.

Keep the first click that requests capture separate from in-game actions. If a
menu opens or closes in the current input batch, do not apply that batch's earlier
raw motion to the newly active camera. All systems still see the same immutable
frame input; this service does not consume events for them.

Headless tests use the same `PointerCapture` intent without claiming an OS lock,
and inject movement with
`InputEvent::relative_pointer_motion(RelativePointerMotion::new(dx, dy)?)`.
Each coordinate and accumulated frame is finite and bounded to an absolute
`RelativePointerMotion::MAX_DISPLACEMENT`. Invalid sums reject BeginFrame before
input or capture intent changes. Desktop collection coalesces only adjacent raw
samples, preserving button boundaries and the configured queue limit. The
[public capture tests](../../tests/pointer_capture.rs) cover these contracts.

Capture requests are immediate application-resource mutations, not deferred
Commands and not rolled back with a failed command batch. As an application
resource, capture intent survives World replacement; release it explicitly when
travelling to a menu or to a World that should not own mouse look.
...
`InputEvent` must handle `RelativePointerMotion` and `FocusLost` as well as
`PointerMoved`, `MouseButton`, and `PointerLeft`. Exhaustive error matches also
need `InputCollectionError::RelativeMotion`, `FrameEdgeLimitExceeded` and,
with desktop enabled, `DesktopRunError::Pointer`.
